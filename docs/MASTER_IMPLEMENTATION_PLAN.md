...
Interview Productization (ADR-028) COMPLETE 2026-08-15: profile-scoped grounding snapshot (resume/JD/role/evidence/prior feedback) injected into question prompt; provenance columns (category/source/source_ref) + 20-category taxonomy; interviewer-reasoning follow-up engine in answer lane (never blocks question stream); deterministic coverage rotation (seeded by session id) + JD gap detection; prep memory (interview_feedback) written at stop() and read by next session; 7 interviewer styles + duration presets in UI; report v2 deterministic scorecard + per-question feedback; anti-hallucination entity guard; migration 0005; 22 new tests; 2 real DeepSeek interviews PASS (grounding + isolation verified); extraction .value bug fixed
Frontend UI freeze (ADR-029) COMPLETE 2026-08-15: Drawing Sheet canon across all 14 routes (9 primary + 5 secondary); More ▾ secondary-records navigation; density refinement (quiet grid, 3-tier rules, annotation micro-labels); DESIGN.md contract written; Impeccable detector clean on all 14 surfaces; commits 88bf5e6 → 90bd43c → c41690b → be87af6
...
| 2026-08 | Frontend visual canon freeze (ADR-029): all 14 routes in the Drawing Sheet language; secondary records (Setup/Report/Transcript/Debrief/Stories) built and committed (90bd43c); More ▾ shell navigation for secondary routes (c41690b); density refinement — quiet grid, three-tier rule hierarchy, annotation micro-labels, stencil weight 500, outlined redline markers (be87af6); DESIGN.md permanent design contract; Impeccable detector clean across all 14 rendered surfaces. | Engineering session |
